...
const Bio = forwardRef<HTMLDivElement, BioProps>(({ className = "", bioMounted = true, languages, ...props }, forwardedRef) => {
    const bioRef = useRef<HTMLDivElement>(null);
    const [chartWidth, setChartWidth] = useState(0);
    const [pieValues, setPieValues] = useState<LanguageDataProp[] | undefined>(undefined);
    ...
    return (
        ...
        >
            <div className="chart">{languages && bioMounted && <LanguageChart pieValues={pieValues} chartWidth={chartWidth} {...props} />}</div>
            <div className="flex flex-col p-5 gap-3">
                <h1 className="about sm:text-3xl md:text-5xl text-left">About Me</h1>
                <p className="desc sm:text-2xl md:text-4xl text-left">
                    I am a 17 year old living in Canada who codes as a hobby. I'm currently studying in high school, and I plan on going into computer science.
                </p>
                <p className="desc sm:text-2xl md:text-4xl text-left mt-5">(more coming soon!)</p>
            </div>
        </div>
    );
});
...
const LanguageChart = forwardRef<HTMLDivElement, BioChartProps>(({ pieValues, chartWidth, className, ...props }, forwardedRef) => {
    const containerRef = useRef<HTMLDivElement>(null);
    const [renderedPieValues, setRenderedPieValues] = useState<LanguageDataProp[]>([]);
